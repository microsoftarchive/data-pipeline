...
namespace Microsoft.Practices.DataPipeline.ColdStorage.BlobWriter
{
    ...
    {
        private static readonly ILogger Logger = LoggerFactory.GetLogger("ColdStorage.RollingBlobWriter");
        ...
        public async Task<bool> WriteAsync(IReadOnlyCollection<BlockData> blocksToWrite, CancellationToken cancellationToken)
        {
            ...
            _instrumentationPublisher.WriteAttempted();
            ...
            Logger.WriteToBlobStarted(_currentBlob.Name, _blockIds.Count, blocksToWrite.Count, totalBytesToWrite);
            ...
            {
                _instrumentationPublisher.WriteFailed();
                Logger.WriteToBlobFailed(ae, _currentBlob.Name, blocksToWrite.Count, totalBytesToWrite);
            ...
            catch (StorageException ex)
            {
                ...
                {
                    _instrumentationPublisher.ConcurrencyFailed();

                    Logger.BlobEtagMissMatchOccured(_currentBlob.Name);
                    ResetCurrentWriterState();
                }
                else if (IsHardStorageException(ex))
                {
                    Logger.HardStorageExceptionCaughtWritingToBlob(ex, _blobClient, _containerName);
                }
                else
                {
                    Logger.StorageExceptionCaughtWritingToBlob(ex, _blobClient, _containerName);
                }
            ...
            Logger.WriteToBlobEnded(_currentBlob.Name, _blockIds.Count);
            _instrumentationPublisher.WriteSucceeded(blocksToWrite.Count, writeTimeStopWatch.Elapsed);
        ...
        private async Task CheckAndRollIfNeededAsync(string prefix, long sizeToWrite, int numChunks, CancellationToken cancellationToken)
        {
            ...
            while ((_sizeRemaining < sizeToWrite) || (_blockIds.Count + numChunks > _blocksAllowed))
            {
                ...
                Logger.RollOccured(currentBlobName, _currentBlob.Name);
            }
        }
        ...
        private bool HandleExceptionsWritingBlocks(AggregateException aggregateException)
        {
            ...
            {
                Logger.HardStorageExceptionCaughtWritingToBlob(storageException, _blobClient, _containerName);
            }
            ...
            foreach (var storageException in softStorageExceptions)
            {
                ...
                {
                    Logger.StorageExceptionCaughtWritingToBlob(storageException, _blobClient, _containerName);
                }
            }
