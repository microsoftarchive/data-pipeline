...
if (-not $fileExists)
{
    ...
    Copy-Item -Path ..\RunFromConsole\mysettings-template.config -Destination ..\RunFromConsole\mysettings.config -Force
}
else
{
    # prompt overwrite
...
if ($continue)
{
    ...
    if(-not (Split-Path $configurationFile -IsAbsolute)){
